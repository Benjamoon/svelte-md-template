<script lang="ts">
	...
	const mainNav = siteConfig.mainNav.filter((item) => !item.external);
...
<Button
	...
	{...$$restProps}
>
	<span class="hidden lg:inline-flex">  {siteConfig.searchPlaceholder} </span>
	<span class="inline-flex lg:hidden"> {siteConfig.searchPlaceholderShort} </span>
	<kbd
